...
function ApplicationTable({
  ...
  return (
    <div className="space-y-4 rounded-3xl mt-4 px-4 pb-4" style={{ backgroundColor: "#1d1b23" }}>
      {isRefreshingTable && <div className="text-center text-white">Loading applications...</div>}
      <Table>
        <TableHeader>
          <TableRow style={{ backgroundColor: "#1d1b23" }}>
            <TableHead onClick={() => handleSort("submission_date")} className="cursor-pointer rounded-tl-2xl rounded-bl-2xl text-white">
              {sortColumn === "submission_date" ? `Submission Date ${sortOrder === "asc" ? "▲" : "▼"}` : "Submission Date ▲▼"}
            </TableHead>
            <TableHead onClick={() => handleSort("team_name")} className="cursor-pointer text-white">
              {sortColumn === "team_name" ? `Team Name ${sortOrder === "asc" ? "▲" : "▼"}` : "Team Name ▲▼"}
            </TableHead>
            <TableHead onClick={() => handleSort("size")} className="cursor-pointer text-white">
              {sortColumn === "size" ? `Size ${sortOrder === "asc" ? "▲" : "▼"}` : "Size ▲▼"}
            </TableHead>
            <TableHead className="relative">
              ...
              <DropdownMenu open={isOpen} onOpenChange={setIsOpen}>
                <DropdownMenuTrigger asChild>
                  <button className="text-white pr-4 py-2 rounded flex items-center justify-between focus:outline-none" onClick={() => setIsOpen((prev) => !prev)}>
                    <span>Status</span>
                    <span className={`ml-2 ${selectedStatus.length > 0 ? "text-[#E75973]" : "text-white"}`}>{isOpen ? "▲" : "▼"}</span>
                  </button>
                </DropdownMenuTrigger>
                <DropdownMenuContent className="max-w-44 bg-black bg-opacity-70" onCloseAutoFocus={(e) => e.preventDefault()}>
                  {/* <DropdownMenuSeparator /> */}
                  {statusOptions.map((status) => (
                    <DropdownMenuCheckboxItem key={status} checked={selectedStatus.includes(status)} onCheckedChange={() => handleSelectStatus(status)} className="text-white">
                      {status}
            ...
            <TableHead className="rounded-tr-2xl rounded-br-2xl">
              ...
              >
                <span className={`${selectedStatus.length === 0 ? "text-gray-400" : "text-white"}`}>Clear</span>
                <span className={`${selectedStatus.length === 0 ? "bg-gray-400 text-gray-200" : "bg-[#E75973] text-white"} rounded-full h-4 w-4 flex items-center justify-center text-[12px]`}>×</span>
              </button>
            </TableHead>
            <TableHead>{}</TableHead>
          </TableRow>
            ...
            currentApplications.map((applicant) => (
              <TableRow key={applicant.application_id} className="bg-[#413F46] text-white">
                <TableCell className="rounded-tl-[0.5rem] rounded-bl-[0.5rem]">
                  {new Date(applicant.submission_date!)
                ...
                <TableCell>
                  <Badge className={`inline-block py-[0.25rem] px-[0.75rem] rounded-[1.0rem] ${statusColors[applicant.status!]}`}>{applicant.status}</Badge>
                </TableCell>
                <TableCell>
                  <Button variant="outline" onClick={() => onViewDetails(applicant)}>
                    View Detail
                  </Button>
                </TableCell>
