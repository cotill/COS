...
export function DropdownFilter({
  ...
  return (
    ...
    >
      ...
      {isOpen && (
        <div
          className="absolute bg-white border rounded shadow p-2 mt-1 z-10 w-full min-w-max"
          style={{
            maxHeight: maxHeight,
            overflowY: 'auto',
          }}
        >
          {options.map((option) => (
            <label key={option} className="block cursor-pointer gap-2 p-1 flex items-center">
              <input
                type="checkbox"
